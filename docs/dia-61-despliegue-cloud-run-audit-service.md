...
Publicar `audit-service` en Artifact Registry, crear el tag operativo `dev` y desplegar el servicio en Cloud Run dev.

Alcance del dia:

```text
Validar prerequisitos de Google Cloud para audit-service.
Publicar la imagen audit-service:0.1.0-native en Artifact Registry.
Promover audit-service:0.1.0-native a audit-service:dev.
Ejecutar preflight de imagen para Cloud Run.
Desplegar audit-service en Cloud Run dev.
...
```text
Artifact Registry contiene audit-service:0.1.0-native.
Artifact Registry contiene audit-service:dev.
Cloud Run desplego audit-service en us-central1.
...
Imagen desplegada: us-central1-docker.pkg.dev/project-fbb34cd7-0b82-43e1-867/venta-pasajes-dev/audit-service:dev
Digest Artifact Registry: sha256:496c5c2831738eaab25eb2005041a338c195c8ad26fb686dd4c63a5a0dc24787
Cloud Run URL: https://audit-service-io7kxgn6yq-uc.a.run.app
Health URL: https://audit-service-io7kxgn6yq-uc.a.run.app/api/v1/audit/health
```
...
Esta reversa deshace el despliegue de Cloud Run de `audit-service` y, si se desea, elimina el tag `dev` creado en Artifact Registry.
...
```powershell
& $GcloudPath run services describe audit-service `
...
Advertencia: este paso elimina el servicio Cloud Run dev `audit-service`.

```powershell
& $GcloudPath run services delete audit-service `
  --project "project-fbb34cd7-0b82-43e1-867" `
  --region "us-central1" `
  --quiet
```
...
& $GcloudPath artifacts docker tags delete `
  "us-central1-docker.pkg.dev/project-fbb34cd7-0b82-43e1-867/venta-pasajes-dev/audit-service:dev" `
  --project "project-fbb34cd7-0b82-43e1-867" `
  --quiet
```
...
```powershell
Remove-Item -LiteralPath "$ProjectRoot\docs\dia-61-despliegue-cloud-run-audit-service.md" -Force
```

Luego retirar las referencias al Dia 61 en:
...
& $GcloudPath iam service-accounts describe `
  "audit-service-run@project-fbb34cd7-0b82-43e1-867.iam.gserviceaccount.com" `
  --project "project-fbb34cd7-0b82-43e1-867" `
  --format "value(email)"

& $GcloudPath secrets describe "audit-service__db-connection" `
  --project "project-fbb34cd7-0b82-43e1-867" `
  --format "value(name)"
```
...
Este paso asume que el Dia 60 ya creo la imagen local y tambien la etiqueta remota local.

```powershell
docker images --format "{{.Repository}}:{{.Tag}} {{.ID}} {{.Size}}" |
  Select-String -Pattern "audit-service|venta-pasajes-dev/audit-service"
```
...
```text
0.1.0-native: digest: sha256:496c5c2831738eaab25eb2005041a338c195c8ad26fb686dd4c63a5a0dc24787
"pushed":true
"ready":true
```
...
```text
sha256:496c5c2831738eaab25eb2005041a338c195c8ad26fb686dd4c63a5a0dc24787
```
...
```text
audit-service backend audit-service 8086 False True
```
...
RoutesReady: True
latestReadyRevisionName: audit-service-00001-7h9
traffic: 100%
...
`audit-service` esta privado, por eso se envia `Authorization: Bearer`.

```powershell
$Url = (& $GcloudPath run services describe audit-service `
  --project "project-fbb34cd7-0b82-43e1-867" `
  --region "us-central1" `
  --format "value(status.url)").Trim()

$Token = (& $GcloudPath auth print-identity-token).Trim()
$HealthUrl = "$Url/api/v1/audit/health"
...
```text
HTTP/1.1 200 OK
{"status":"ok","service":"audit-service","runtime":"quarkus","checked_at":"2026-09-21T15:42:17.173294Z"}
HTTP_STATUS:200
```
...
& $GcloudPath iam service-accounts describe "audit-service-run@project-fbb34cd7-0b82-43e1-867.iam.gserviceaccount.com" --project project-fbb34cd7-0b82-43e1-867 --format "value(email)"
& $GcloudPath secrets describe "audit-service__db-connection" --project project-fbb34cd7-0b82-43e1-867 --format "value(name)"
...
Service account audit-service-run existe.
Secret audit-service__db-connection existe.
audit-service:0.1.0-native publicado con digest sha256:496c5c2831738eaab25eb2005041a338c195c8ad26fb686dd4c63a5a0dc24787.
audit-service:dev creado y apunta al mismo digest.
...
```text
audit-service queda privado en Cloud Run dev.
La validacion de health requiere token de identidad.
El servicio mantiene su base propia audit_db.
El servicio no debe consultar bases de otros microservicios directamente.
El siguiente bloque operativo puede enfocarse en empaquetar y desplegar frontends/MFEs.
```
