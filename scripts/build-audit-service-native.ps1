param(
    ...
    [string]$ImageName = "audit-service",
...
$ServiceRoot = Join-Path -Path $Root -ChildPath "services\audit-service"
$BuildWorkspace = $null
$EffectiveServiceRoot = $ServiceRoot

if ($UseCleanWorkspace) {
    $BuildWorkspace = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "venta-pasajes-audit-native-$PID"
    $EffectiveServiceRoot = Join-Path -Path $BuildWorkspace -ChildPath "audit-service"
...
if (-not (Test-Path -LiteralPath $PomPath)) {
    throw "audit-service pom.xml was not found: $PomPath"
}
