param(
    ...
    [string]$ImageName = "ticketing-service",
...
if ($UseCleanWorkspace) {
    $BuildWorkspace = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "venta-pasajes-ticketing-native-$PID"
    $EffectiveServiceRoot = Join-Path -Path $BuildWorkspace -ChildPath "ticketing-service"
...
if (-not (Test-Path -LiteralPath $PomPath)) {
    throw "ticketing-service pom.xml was not found: $PomPath"
}
