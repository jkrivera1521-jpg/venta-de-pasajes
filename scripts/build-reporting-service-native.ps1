param(
    ...
    [string]$ImageName = "reporting-service",
...
$ServiceRoot = Join-Path -Path $Root -ChildPath "services\reporting-service"
$BuildWorkspace = $null
$EffectiveServiceRoot = $ServiceRoot

if ($UseCleanWorkspace) {
    $BuildWorkspace = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "venta-pasajes-reporting-native-$PID"
    $EffectiveServiceRoot = Join-Path -Path $BuildWorkspace -ChildPath "reporting-service"
...
function Assert-DockerAvailable {
    ...
    if ($DockerExitCode -ne 0) {
        throw "Docker Desktop is not running or is not reachable. Start Docker Desktop and wait until it says Running before executing this script."
    }
}

if (-not (Test-Path -LiteralPath $PomPath)) {
    throw "reporting-service pom.xml was not found: $PomPath"
}
...
if ($PlanOnly) {
    [pscustomobject]@{
        ...
        push = [bool]$Push
        create_repository = [bool]$CreateRepository
        ready = $true
...
if (-not $SkipNativeBuild -or -not $SkipDockerBuild) {
...
[pscustomobject]@{
    service = "reporting-service"
    native_runner = $(if ($NativeRunner) { $NativeRunner.FullName } else { $null })
    native_runner_bytes = $(if ($NativeRunner) { $NativeRunner.Length } else { $null })
