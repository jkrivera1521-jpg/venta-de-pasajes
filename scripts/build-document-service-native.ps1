param(
    ...
    [string]$ImageName = "document-service",
...
$ServiceRoot = Join-Path -Path $Root -ChildPath "services\document-service"
$BuildWorkspace = $null
$EffectiveServiceRoot = $ServiceRoot

if ($UseCleanWorkspace) {
    $BuildWorkspace = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "venta-pasajes-document-native-$PID"
    $EffectiveServiceRoot = Join-Path -Path $BuildWorkspace -ChildPath "document-service"
...
if (-not (Test-Path -LiteralPath $PomPath)) {
    throw "document-service pom.xml was not found: $PomPath"
}
