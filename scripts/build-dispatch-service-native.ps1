param(
    ...
    [string]$ImageName = "dispatch-service",
...
if ($UseCleanWorkspace) {
    $BuildWorkspace = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "venta-pasajes-dispatch-native-$PID"
    $EffectiveServiceRoot = Join-Path -Path $BuildWorkspace -ChildPath "dispatch-service"
...
if (-not (Test-Path -LiteralPath $PomPath)) {
    throw "dispatch-service pom.xml was not found: $PomPath"
}
