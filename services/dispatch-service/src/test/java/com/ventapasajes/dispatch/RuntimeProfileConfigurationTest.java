package com.ventapasajes.dispatch;
...
class RuntimeProfileConfigurationTest {
    ...
    void applicationPropertiesDeclareGcpAndOnPremiseProfiles() throws IOException {
        ...
        assertContains(properties, "%gcp.quarkus.datasource.username=${APP_DB_USERNAME:dispatch-service-run@project-fbb34cd7-0b82-43e1-867.iam}");
        ...
        assertContains(properties, "%onprem.quarkus.datasource.jdbc.url=${APP_DB_JDBC_URL:jdbc:postgresql://localhost:5432/dispatch_db}");
        assertContains(properties, "quarkus.flyway.baseline-on-migrate=${QUARKUS_FLYWAY_BASELINE_ON_MIGRATE:false}");
        assertContains(properties, "quarkus.native.resources.includes=db/migration/*.sql");
    }
